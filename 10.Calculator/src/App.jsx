...
function reducer(state, action) {
  switch (action.type) {
    case "ADD_DIGIT":
      // If we just calculated a result, start new input
      if (state.overwrite) {
        return {
          ...state,
    ...
    case "CHOOSE_OPERATOR":
      // If an operator already exists and we are not overwriting, calculate intermediate result
      if (state.operator && !state.overwrite) {
        const intermediate = evaluate(state);
        return {
          ...state,
